#!/usr/bin/env python3
"""
Add AcademiaObscura tag to all posts that have any of the following tags:
- funny
- cats
- listicles
- twitter
...
TARGET_TAGS = ['funny', 'cats', 'listicles', 'twitter', 'chemistry']

def has_target_tag(frontmatter_text):
    """Check if frontmatter has any of the target tags."""
    tags_match = re.search(r'^tags:\s*(.*?)$', frontmatter_text, re.MULTILINE)
    if tags_match:
        tags_value = tags_match.group(1).strip()
        for tag in TARGET_TAGS:
            if tag in tags_value:
                return True
    return False
...
def process_file(file_path):
    ...
    # Check if it has any target tag
    if not has_target_tag(frontmatter_text):
        return False, "Does not have any target tags"
...
def main():
    ...
    print(f"Looking for posts with tags: {', '.join(TARGET_TAGS)}")
    print("=" * 60)
