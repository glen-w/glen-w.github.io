---
layout: library-item
title: 'Regional Ocean Governance of Areas Beyond National Jurisdiction: Lessons Learnt
  and Ways Forward'
date: '2019-01-01'
entry_type: Report
year: '2019'
bibtex_key: wrightRegionalOceanGovernance2019
is_event: false
description: 'Regional Ocean Governance of Areas Beyond National Jurisdiction: Lessons
  Learnt and Ways Forward'
tags:
- bbnj
- not undermine
- ifb
- oa:unknown
- author:glen wright
categories:
- other
authors:
- Glen Wright
- Julien Rochette
venue: STRONG High Seas
institution: STRONG High Seas
role: lead author
preview: glen_wright_etal_2019_regional_ocean_governance_areas_beyond_national_jurisdiction_lessons_learnt_ways_forward
pdf: glen_wright_etal_2019_regional_ocean_governance_areas_beyond_national_jurisdiction_lessons_learnt_ways_forward_b.pdf
resources:
- kind: pdf
  title: 'Regional Ocean Governance of Areas Beyond National Jurisdiction: Lessons
    Learnt and Ways Forward'
  label: View report PDF
  url: /assets/pdf/glen_wright_etal_2019_regional_ocean_governance_areas_beyond_national_jurisdiction_lessons_learnt_ways_forward_b.pdf
  format: PDF
  local: true
  external: false
  primary: true
---

