---
layout: library-item
title: Opportunities for Strengthening Ocean Governance in the Southeast Pacific
date: '2018-01-01'
entry_type: Workshop
year: '2018'
bibtex_key: OpportunitiesStrengtheningOcean2018
is_event: true
description: Opportunities for Strengthening Ocean Governance in the Southeast Pacific
categories:
- other
location: Cali, Colombia
role: speaker
preview: 2018_opportunities_strengthening_ocean_governance_southeast_pacific
gallery:
- opportunities_strengthening_ocean_governance_southeast_pacific_photo_01
- opportunities_strengthening_ocean_governance_southeast_pacific_photo_02
- opportunities_strengthening_ocean_governance_southeast_pacific_photo_03
pdf: 2018_opportunities_strengthening_ocean_governance_southeast_pacific_a.pdf
zip_archive: 2018_opportunities_strengthening_ocean_governance_southeast_pacific.zip
zip_file_count: '4'
zip_file_size_mb: '18.1'
resources:
- kind: pdf
  title: Opportunities for Strengthening Ocean Governance in the Southeast Pacific
  label: View primary document
  url: /assets/pdf/2018_opportunities_strengthening_ocean_governance_southeast_pacific_a.pdf
  format: PDF
  local: true
  external: false
  primary: true
- kind: zip
  title: Download all files
  label: Download all files
  url: /assets/zips/2018_opportunities_strengthening_ocean_governance_southeast_pacific.zip
  format: ZIP
  local: true
  external: false
  subtitle: 4 files · 18.1 MB
---

