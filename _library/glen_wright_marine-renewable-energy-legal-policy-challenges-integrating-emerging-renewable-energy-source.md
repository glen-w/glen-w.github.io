---
layout: library-item
title: "Marine Renewable Energy: Legal and Policy Challenges to Integrating an Emerging
  Renewable Energy Source"
date: "2012-01-01"
entry_type: Conference
year: "2012"
bibtex_key: Wright2012a
is_event: true
description: "Marine Renewable Energy: Legal and Policy Challenges to Integrating
  an Emerging Renewable Energy Source"
categories:
  - other
authors:
  - Glen Wright
venue: IKEM International Summer Academy on Energy and the Environment
location: Berlin, Germany
role: speaker
preview: glen_wright_2012_marine_renewable_energy_legal_policy_challenges_integrating_emerging_renewable_energy_source
gallery:
  - glen_wright_2012_marine_renewable_energy_legal_policy_challenges_integrating_emerging_renewable_energy_source_photo_01
  - glen_wright_2012_marine_renewable_energy_legal_policy_challenges_integrating_emerging_renewable_energy_source_photo_02
pdf: glen_wright_2012_marine_renewable_energy_legal_policy_challenges_integrating_emerging_renewable_energy_source.pdf
url: https://www.ikem.de/en/summer-academy-2012/
zip_archive: glen_wright_2012_marine_renewable_energy_legal_policy_challenges_integrating_emerging_renewable_energy_source_a.zip
zip_file_count: "3"
zip_file_size_mb: "6.4"
number: July
resources:
  - kind: pdf
    title: "Marine Renewable Energy: Legal and Policy Challenges to Integrating an Emerging
      Renewable Energy Source"
    label: View primary document
    url: /assets/pdf/glen_wright_2012_marine_renewable_energy_legal_policy_challenges_integrating_emerging_renewable_energy_source.pdf
    format: PDF
    local: true
    external: false
    primary: true
  - kind: zip
    title: Download all files
    label: Download all files
    url: /assets/zips/glen_wright_2012_marine_renewable_energy_legal_policy_challenges_integrating_emerging_renewable_energy_source_a.zip
    format: ZIP
    local: true
    external: false
    subtitle: 3 files · 6.4 MB
  - kind: landing
    title: Visit IKEM International Summer Academy on Energy and the Environment publication
      page
    label: Visit IKEM International Summer Academy on Energy and the Environment publication
      page
    url: https://www.ikem.de/en/summer-academy-2012/
    format: Web
    local: false
    external: true
---
