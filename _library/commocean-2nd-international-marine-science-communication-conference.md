---
layout: library-item
title: 'CommOcean: 2nd International Marine Science Communication Conference'
date: '2016-12-01'
entry_type: Conference
year: '2016'
bibtex_key: CommOcean2ndInternational2016
is_event: true
description: 'CommOcean: 2nd International Marine Science Communication Conference'
categories:
- other
location: Bruges, Belgium
role: attendee
preview: 2016_commocean_2nd_international_marine_science_communication_conference
gallery:
- commocean_2nd_international_marine_science_communication_conference_figure_01
- commocean_2nd_international_marine_science_communication_conference_figure_02
- commocean_2nd_international_marine_science_communication_conference_photo_01
agenda: 2016_commocean_2nd_international_marine_science_communication_conference_agenda.pdf
zip_archive: 2016_commocean_2nd_international_marine_science_communication_conference.zip
zip_file_count: '4'
zip_file_size_mb: '9.3'
resources:
- kind: agenda
  title: Conference agenda
  label: View conference programme
  url: /assets/pdf/2016_commocean_2nd_international_marine_science_communication_conference_agenda.pdf
  format: PDF
  local: true
  external: false
- kind: zip
  title: Download all files
  label: Download all files
  url: /assets/zips/2016_commocean_2nd_international_marine_science_communication_conference.zip
  format: ZIP
  local: true
  external: false
  subtitle: 4 files · 9.3 MB
---

