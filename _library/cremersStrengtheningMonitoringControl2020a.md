---
abstract: Monitoring, control and surveillance (MCS) is critical for the success of
  marine conservation and management, but effective MCS remains challenging. This
  is especially true for the deep and distant waters of marine areas beyond national
  jurisdiction (ABNJ), which are characterised by a fragmented governance framework
  and reliance on flag States to ensure control over vessels. This report explores
  how the BBNJ negotiations can strengthen MCS in ABNJ and highlights how MCS can
  contribute to the implementation of the new instrument. The report provides an overview
  of existing legal frameworks, technological tools and MCS initiatives, and highlights
  some of the challenges to effective MCS. The report provides some suggested pathways
  for strengthening MCS in ABNJ, as well as three concrete proposals for provisions
  that could be included in the future international instrument.
authors:
- Klaudija Cremers
- Glen Wright
- Julien Rochette
bibtex_key: cremersStrengtheningMonitoringControl2020a
categories:
- other
date: '2020-01-01'
description: Monitoring, control and surveillance (MCS) is critical for the success
  of marine conservation and management, but effective MCS remains challenging. This
  is especially true for the deep and distant wa...
entry_type: Other
institution: STRONG High Seas
layout: library-item
pages: '46'
pdf: klaudija_cremers_2020_strengthening_monitoring_control_surveillance.pdf
preview: klaudija_cremers_2020_strengthening_monitoring_control_surveillance.jpeg
tags: []
title: Strengthening Monitoring, Control and Surveillance in Areas Beyond National
  Jurisdiction
venue: STRONG High Seas
year: '2020'
---

## Abstract

Monitoring, control and surveillance (MCS) is critical for the success of marine conservation and management, but effective MCS remains challenging. This is especially true for the deep and distant waters of marine areas beyond national jurisdiction (ABNJ), which are characterised by a fragmented governance framework and reliance on flag States to ensure control over vessels. This report explores how the BBNJ negotiations can strengthen MCS in ABNJ and highlights how MCS can contribute to the implementation of the new instrument. The report provides an overview of existing legal frameworks, technological tools and MCS initiatives, and highlights some of the challenges to effective MCS. The report provides some suggested pathways for strengthening MCS in ABNJ, as well as three concrete proposals for provisions that could be included in the future international instrument.

## Publication Details

**Authors:** Klaudija Cremers, Glen Wright, Julien Rochette

**Venue:** STRONG High Seas
**Year:** 2020
**Pages:** 46
**Institution:** STRONG High Seas

## Links and Resources

- [PDF](klaudija_cremers_2020_strengthening_monitoring_control_surveillance.pdf)
- [Additional Link](klaudija_cremers_2020_strengthening_monitoring_control_surveillance.jpeg)
