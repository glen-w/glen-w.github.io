---
layout: library-item
title: Regional Ocean Governance  Conservation and Sustainable Use of Marine Biodiversity
date: '2015-10-01'
entry_type: Workshop
year: '2015'
bibtex_key: durusselRegionalOceanGovernance2015
is_event: true
description: Regional Ocean Governance  Conservation and Sustainable Use of Marine
  Biodiversity
categories:
- other
authors:
- Carole Durussel
- Glen Wright
venue: CPPS Workshop on Integrated Regional Oceanic Policy
publisher: IDDRI, IASS
role: speaker
preview: 2015_regional_ocean_governance_conservation_sustainable_use_marine_biodiversity
gallery:
- regional_ocean_governance_conservation_sustainable_use_marine_biodiversity_figure_01
pdf: 2015_regional_ocean_governance_conservation_sustainable_use_marine_biodiversity.pdf
slides: 2015_regional_ocean_governance_conservation_sustainable_use_marine_biodiversity_a_slides.pdf
zip_archive: 2015_regional_ocean_governance_conservation_sustainable_use_marine_biodiversity_a.zip
zip_file_count: '3'
zip_file_size_mb: '4.2'
resources:
- kind: pdf
  title: Regional Ocean Governance  Conservation and Sustainable Use of Marine Biodiversity
  label: View primary document
  url: /assets/pdf/2015_regional_ocean_governance_conservation_sustainable_use_marine_biodiversity.pdf
  format: PDF
  local: true
  external: false
  primary: true
- kind: slides
  title: Presentation slides
  label: View presentation
  url: /assets/pdf/2015_regional_ocean_governance_conservation_sustainable_use_marine_biodiversity_a_slides.pdf
  format: PDF
  local: true
  external: false
- kind: zip
  title: Download all files
  label: Download all files
  url: /assets/zips/2015_regional_ocean_governance_conservation_sustainable_use_marine_biodiversity_a.zip
  format: ZIP
  local: true
  external: false
  subtitle: 3 files · 4.2 MB
---

