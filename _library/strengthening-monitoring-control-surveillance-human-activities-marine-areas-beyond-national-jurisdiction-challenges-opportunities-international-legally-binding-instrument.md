---
layout: library-item
title: "Strengthening monitoring, control and surveillance of human activities in
  marine areas beyond national jurisdiction: Challenges and opportunities for an international
  legally binding instrument"
date: "2020-12-01"
entry_type: Journal article
year: "2020"
bibtex_key: cremersStrengtheningMonitoringControl2020
is_event: false
description: Monitoring, control and surveillance (MCS) is critical for the success
  of marine conservation and management.
standfirst: Monitoring, control and surveillance (MCS) is critical for the success
  of marine conservation and management.
abstract: Monitoring, control and surveillance (MCS) is critical for the success of
  marine conservation and management. This raises specific challenges in the deep
  and distant waters of marine areas beyond national jurisdiction (ABNJ), which is
  characterised by a fragmented governance framework and reliance on flag States to
  ensure control over vessels. States at the United Nations are currently negotiating
  an international legally binding instrument for the conservation and sustainable
  use of the biological diversity of marine areas beyond national jurisdiction and
  there is a growing interest in how MCS tools and policies can contribute to the
  management of this vast global commons. The paper provides some suggested pathways
  for strengthening MCS in ABNJ, as well as three concrete proposals for provisions
  that could be included in the future international instrument.
categories:
  - other
authors:
  - Klaudija Cremers
  - Glen Wright
  - Julien Rochette
venue: Marine Policy
role: co-author
preview: 2020_strengthening_monitoring_control_surveillance_human_activities_marine_areas_beyond_national_jurisdiction_challenges_opportunities_international_legally_binding_instrument
pdf: 2020_strengthening_monitoring_control_surveillance_human_activities_marine_areas_beyond_national_jurisdiction_challenges_opportunities_international_legally_binding_instrument.pdf
url: https://www.sciencedirect.com/science/article/pii/S0308597X19309534
doi: https://doi.org/10.1016/j.marpol.2020.103976
zip_archive: 2020_strengthening_monitoring_control_surveillance_human_activities_marine_areas_beyond_national_jurisdiction_challenges_opportunities_international_legally_binding_instrument_a.zip
zip_file_count: "1"
zip_file_size_mb: "0.3"
pages: "103976"
volume: "122"
resources:
  - kind: pdf
    title: "Strengthening monitoring, control and surveillance of human activities in
      marine areas beyond national jurisdiction: Challenges and opportunities for an
      international legally binding instrument"
    label: View primary document
    url: /assets/pdf/2020_strengthening_monitoring_control_surveillance_human_activities_marine_areas_beyond_national_jurisdiction_challenges_opportunities_international_legally_binding_instrument.pdf
    format: PDF
    local: true
    external: false
    primary: true
  - kind: zip
    title: Download all files
    label: Download all files
    url: /assets/zips/2020_strengthening_monitoring_control_surveillance_human_activities_marine_areas_beyond_national_jurisdiction_challenges_opportunities_international_legally_binding_instrument_a.zip
    format: ZIP
    local: true
    external: false
    subtitle: 1 files · 0.3 MB
  - kind: landing
    title: Visit Marine Policy publication page
    label: Visit Marine Policy publication page
    url: https://www.sciencedirect.com/science/article/pii/S0308597X19309534
    format: Web
    local: false
    external: true
---
